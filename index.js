...
export async function user(username: string, session?: string): Promise<InstagramApiResult["graphql"]["user"] | null> {
    if (!username) return null;
    try { 
        const apiresult = await fetch<InstagramApiResult>(`https://instagram.com/${userName}${session ? "" : "/feed"}/?__a=1`,{ headers: session ? { cookie: `sessionid=${session}` } : { } }, FetchResultTypes.JSON);
